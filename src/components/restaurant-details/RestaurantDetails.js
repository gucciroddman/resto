...
const getCombinedCategoriesAndProducts = (
    ...
    const categoryIdInRestaurant = allCategories?.filter((item) =>
        restaurantCategoryIds?.includes(item.id)
    )

    if (categoryIdInRestaurant?.length > 0 && allProducts?.length > 0) {
        const data = categoryIdInRestaurant?.map((item) => {
